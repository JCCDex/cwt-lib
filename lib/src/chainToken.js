...
function getAlg(chain, key, keyType) {
    switch (chain) {
        case "ethereum":
            return "secp256k1";
        case "ripple":
...
class ChainToken {
    ...
    static quickSign(key, usr, chain) {
        if (!utils_1.CHAIN_USE_KEY.has(chain)) {
            throw new Error(`chain ${chain} is not supported`);
        }
        const alg = getAlg(chain, key, utils_1.CHAIN_USE_KEY.get(chain).keyType);
        const tokenItem = new ChainToken(chain, key, utils_1.CHAIN_USE_KEY.get(chain).sign);
        const data = {
            header: {
